...
static void button_sir(const struct device *dev, struct gpio_callback *cb,
                        uint32_t pins)
{
    ARG_UNUSED(dev);
    ARG_UNUSED(cb);
...
static void cycle_thread_fn(void *a, void *b, void *c)
{
    ...
    while (1) {
        ...
        gps_fix_t fix;
        bool_got_fix = gps_acquire_fix(&fix, GPS_FIX_TIMEOUT_MS);
...
void state_machine_init(void)
{
    ...
    gpio_init_callback(&m_btn_cb, button_sir, BIT(m_btn.pin));
